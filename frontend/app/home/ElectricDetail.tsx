...
export default function ElectricDetail() {
  ...
  useEffect(() => {
    fetch("http://192.168.0.112:5000/api/electric")
      .then((res) => res.json())
  ...
  return (
    <View className="flex-1 items-center justify-center bg-white px-4 w-full">
      <Text className="text-xl font-bold mb-4 mt-4">Electric Data</Text>
      <FlatList
        className="w-full"
        data={currentData}
        ...
        renderItem={({ item }) => (
          <View className="p-4 border-b border-gray-200 w-full flex-row items-center justify-between">
            ...
            <View className="p-4">
              <Text className="text-base">⚡️ Amount:    {item.kwh} kwh</Text>
            </View>
          </View>
        )}
      />
