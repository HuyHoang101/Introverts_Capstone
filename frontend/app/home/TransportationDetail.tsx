...
export default function TransportationDetail() {
  ...
  useEffect(() => {
    fetch("http://192.168.0.103:5000/api/transportation")
      .then((res) => res.json())
  ...
  return (
    <View className="flex-1 items-center justify-center bg-white px-4 w-full">
      <Text className="text-xl font-bold mb-4 mt-4">Transportation Data</Text>
      <FlatList
        className="w-full"
        data={currentData}
        ...
        renderItem={({ item }) => (
          <View className="p-4 border-b border-gray-200 w-full flex-row items-center justify-between">
            ...
            <View className="p-4">
              <Text className="text-base">🚲 Bike:              {item.bike} devices</Text>
              <Text className="text-base">🛵 Motorbike:    {item.motorbike} devices</Text>
              <Text className="text-base">🚗 Car:               {item.car} devices</Text>
            </View>
          </View>
        )}
      />
