...
import { getAllWaterData, deleteWaterData } from "@/service/waterService";
...
const WaterList = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await getAllWaterData();
      setData(res);
    } catch (err) {
  ...
  const confirmDelete = (id: string) => {
    Alert.alert("Are you sure?", "This will permanently delete the record.", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await deleteWaterData(id);
            setData((prev) => prev.filter((item) => item.id !== id));
          } catch (err) {
            console.error("Failed to delete water data:", err);
          }
        },
      },
    ]);
  ...
  return (
    ...
    >
      ...
      <View className="flex-row items-center justify-between px-4 pt-12 pb-4">
        ...
        <Text className="text-2xl font-bold text-white">Water Usage</Text>
        <View className="flex-row">
          <TouchableOpacity
            onPress={() => router.push("/data/addWater")}
